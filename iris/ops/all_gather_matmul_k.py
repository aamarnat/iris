...
"""
Fused All-Gather (along K) + GEMM operation using pull pattern.

Each rank has a column-sharded input A_sharded (M x K_local).
This operation computes C = all_gather_k(A_sharded) @ B by pulling
...
@triton.jit()
def _fused_all_gather_matmul_k_kernel(
    A_sharded,
    B,
    C,
    bias_ptr,
    ...
    """Fused all-gather (along K) + GEMM kernel using pull pattern."""
    # ═══════════════════════════════════════════════════════════════════════
...
def all_gather_matmul_k_preamble(
    shmem,
    A_sharded: torch.Tensor,
    B: torch.Tensor,
    config: Optional[FusedConfig] = None,
) -> FusedWorkspace:
    """Allocate workspace for all_gather_matmul_k (none needed for pull pattern)."""
    if config is None:
        config = FusedConfig()
...
def all_gather_matmul_k(
    ...
    """Fused all-gather (along K) and matrix multiplication using pull pattern."""
    if config is None:
        config = FusedConfig()
    ...
    if workspace is None:
        workspace = all_gather_matmul_k_preamble(shmem, A_sharded, B, config)
    ...
    grid = (num_sms,)
    iris_launch(
        _fused_all_gather_matmul_k_kernel,
        grid,
        A_sharded,
        B,
        output_tensor,
